...
def get_reason_model_bundle(model_name=DEFAULT_REASON_MODEL_NAME):
    ...
    try:
        ...
        message = str(exc)
        if "gated repo" in message.lower() or "401" in message or "access to model" in message.lower():
            raise RuntimeError(
                "Cosmos Reason 2 requires Hugging Face gated-model access. "
                "Make sure your account has access to nvidia/Cosmos-Reason2-8B and that HF_TOKEN "
                "or HUGGINGFACE_HUB_TOKEN is set to a fresh token before launching Streamlit."
            ) from exc
        raise
